...
export default function AdminPage() {
  ...
  // LOGIKA LOGIN TEMBAK KE API VERCEL
      ...
      if (match) {
        ...
        fm.split('\n').forEach(line => {
          ...
          if (line.startsWith('images:')) {
             try { imgs = JSON.parse(line.replace('images:', '').trim()); } catch(e){}
          }
        });
    ...
    try {
      ...
      if (fileImages.length > 0) {
         ...
         for (let i = 0; i < fileImages.length; i++) {
            ...
            const res = await fetch('/api/publish', {
               method: 'POST',
               body: JSON.stringify({ action: 'upload_image', filename: fileImages[i].name, base64: b64 })
            });
      ...
      const res = await fetch('/api/publish', {
        ...
        body: JSON.stringify({
          ...
          penulis: `${loggedInUser.name} | ${loggedInUser.role}` // OTOMATIS NAMBAH NAMA PENULIS
        })
      });
